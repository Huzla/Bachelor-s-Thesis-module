...
void mf_wa(int ny, int nx, int hy, int hx, const float* in, float* out) {
	...
	for (int i = 0; i < nx*ny; ++i)
	{
		...
		{
			int row = nx*median_y;
			for (int median_x = median_x_start + row; median_x <= median_x_end + row;
++median_x)
			{
		...
		{
			double temp = 0;
			std::nth_element(median_vector.begin(), median_vector.begin() + size/2-1,
			median_vector.begin() + size);
			temp += median_vector[size/2-1];
			std::nth_element(median_vector.begin(), median_vector.begin() + size/2,
			median_vector.begin() + size);
